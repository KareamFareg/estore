...
class TopSellingItemsComponent extends Component
{
    use WithPagination;
    use wishListTrait;
    use cartTrait;
    ...
    {
        $sale=Sale::find(1);
        $products =  Product::where('sale_price','>',0)->orderBy('ordered_count','DESC')->paginate(12);

        if (Auth::check()){
            Cart::instance('cart')->store(Auth::user()->email);
            Cart::instance('wishlist')->store(Auth::user()->email);
        }
        return view('livewire.top-selling-items-component',['products'=>$products,'sale'=>$sale])->layout('layouts.base');
    }
}
